...
class GainMCSampler:
    ...
    def build_gain_model(self,
        ...
        with pm.Model() as model:
            ...
            log_base_amp_xx = pm.Normal('log_base_amp_xx',
                                       mu=np.log(base_amp_mean),
                                       sigma=base_amp_std,
                                       shape=self.n_antennas)
            log_base_amp_yy = pm.Normal('log_base_amp_yy',
            ...
            base_phase_xx = pm.Normal('base_phase_xx', mu=0.0, sigma=phase_std,
                                     shape=self.n_antennas)
            base_phase_yy = pm.Normal('base_phase_yy', mu=0.0, sigma=phase_std,
                                     shape=self.n_antennas)

            # Thermal drift parameters per antenna
            thermal_amp_xx = pm.Normal('thermal_amp_xx', mu=0.0, sigma=thermal_amplitude,
                                      shape=self.n_antennas)
            thermal_amp_yy = pm.Normal('thermal_amp_yy', mu=0.0, sigma=thermal_amplitude,
                                      shape=self.n_antennas)

            thermal_phase_xx = pm.Uniform('thermal_phase_xx', lower=0, upper=2*np.pi,
                                         shape=self.n_antennas)
            thermal_phase_yy = pm.Uniform('thermal_phase_yy', lower=0, upper=2*np.pi,
                                         shape=self.n_antennas)

            # Phase drift rates (rad/second)
            phase_drift_rate_xx = pm.Normal('phase_drift_rate_xx', mu=0.0, sigma=1e-5,
                                           shape=self.n_antennas)
            phase_drift_rate_yy = pm.Normal('phase_drift_rate_yy', mu=0.0, sigma=1e-5,
                                           shape=self.n_antennas)
            ...
            thermal_mod_xx = thermal_amp_xx[:, None] * pt.sin(
                2 * np.pi * time_tensor[None, :] / thermal_timescale + thermal_phase_xx[:, None]
            )
            thermal_mod_yy = thermal_amp_yy[:, None] * pt.sin(
                2 * np.pi * time_tensor[None, :] / thermal_timescale + thermal_phase_yy[:, None]
            )

            # Phase evolution
            phase_evolution_xx = (base_phase_xx[:, None] +
                                 phase_drift_rate_xx[:, None] * time_tensor[None, :])
            phase_evolution_yy = (base_phase_yy[:, None] +
            ...
            gains_xx = pm.Deterministic('gains_xx',
                                       amp_xx * pt.exp(1j * phase_evolution_xx))
            gains_yy = pm.Deterministic('gains_yy',
                                       amp_yy * pt.exp(1j * phase_evolution_yy))
    ...
    def extract_gain_samples(self, antenna_id: int = 0) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gains_xx = self.trace.posterior['gains_xx'].values[:, :, antenna_id, :]  # (chain, draw, time)
    ...
    def plot_gain_evolution(self,
        ...
        for i in range(n_samples):
            amp_fig.line(times, np.abs(gains_xx[i]), alpha=0.1, color='blue', line_width=1)
            amp_fig.line(times, np.abs(gains_yy[i]), alpha=0.1, color='red', line_width=1)

            phase_fig.line(times, np.degrees(np.angle(gains_xx[i])), alpha=0.1, color='blue', line_width=1)
            phase_fig.line(times, np.degrees(np.angle(gains_yy[i])), alpha=0.1, color='red', line_width=1)
        ...
        if show_percentiles:
            ...
            phase_xx_percentiles = np.percentile(np.degrees(np.angle(gains_xx)), [5, 50, 95], axis=0)
            phase_yy_percentiles = np.percentile(np.degrees(np.angle(gains_yy)), [5, 50, 95], axis=0)

            # Fill between percentiles
            amp_fig.varea(x=times, y1=amp_xx_percentiles[0], y2=amp_xx_percentiles[2],
                         alpha=0.3, color='blue', legend_label='XX 90% CI')
            amp_fig.varea(x=times, y1=amp_yy_percentiles[0], y2=amp_yy_percentiles[2],
                         alpha=0.3, color='red', legend_label='YY 90% CI')

            phase_fig.varea(x=times, y1=phase_xx_percentiles[0], y2=phase_xx_percentiles[2],
                           alpha=0.3, color='blue', legend_label='XX 90% CI')
            phase_fig.varea(x=times, y1=phase_yy_percentiles[0], y2=phase_yy_percentiles[2],
                           alpha=0.3, color='red', legend_label='YY 90% CI')

            # Plot medians
            amp_fig.line(times, amp_xx_percentiles[1], color='blue', line_width=3,
                        legend_label='XX median')
            amp_fig.line(times, amp_yy_percentiles[1], color='red', line_width=3,
                        legend_label='YY median')

            phase_fig.line(times, phase_xx_percentiles[1], color='blue', line_width=3,
                          legend_label='XX median')
            phase_fig.line(times, phase_yy_percentiles[1], color='red', line_width=3,
                          legend_label='YY median')
...
def create_gain_example(n_antennas: int = 4, n_times: int = 50) -> GainMCSampler:
    ...
    print(f"Sampling complete. Summary:")
