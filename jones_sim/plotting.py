...
class JonesPlotter:
    ...
    def plot_jones_matrix_elements(self,
                                   jones_matrices: np.ndarray,
                                   frequencies: np.ndarray,
                                   times: np.ndarray,
                                   antenna_id: int = 0,
                                   title_prefix: str = "Jones Matrix") -> List:
        ...
        for i in range(2):
            for j in range(2):
                ...
                amp_fig.image(image=[amp_data], x=frequencies.min(), y=times.min(),
                ...
                phase_mapper = LinearColorMapper(palette=RdYlBu11,
                                               low=-np.pi, high=np.pi)

                phase_fig.image(image=[phase_data], x=frequencies.min(), y=times.min(),
                               dw=frequencies.max()-frequencies.min(),
                               dh=times.max()-times.min(),
    ...
    def plot_effect_vs_frequency(self,
                                effect_instance,
                                frequencies: np.ndarray,
                                time: float = 0.0,
                                antenna_id: int = 0) -> List:
        ...
        for i in range(2):
            for j in range(2):
                ...
                amp_fig.circle(frequencies, np.abs(element), size=4, color='blue', alpha=0.6)
                ...
                phase_fig.line(frequencies, np.angle(element), line_width=2, color='red')
                phase_fig.circle(frequencies, np.angle(element), size=4, color='red', alpha=0.6)

                figures.extend([amp_fig, phase_fig])

        return figures

    def plot_effect_vs_time(self,
                           effect_instance,
                           times: np.ndarray,
                           frequency: float = 1e9,
                           antenna_id: int = 0) -> List:
        ...
        for i in range(2):
            for j in range(2):
                ...
                phase_fig.circle(times, np.angle(element), size=4, color='orange', alpha=0.6)

                figures.extend([amp_fig, phase_fig])

        return figures

    def plot_visibility_corruption(self,
                                  ideal_visibilities: np.ndarray,
                                  corrupted_visibilities: np.ndarray,
                                  frequencies: np.ndarray,
                                  baseline_names: Optional[List[str]] = None) -> List:
        ...
        for corr_idx, corr_name in enumerate(correlation_names):
            ...
            amp_fig.line(frequencies, np.abs(ideal), legend_label="Ideal",
                        line_width=2, color='blue')
            amp_fig.line(frequencies, np.abs(corrupted), legend_label="Corrupted",
            ...
            phase_fig.line(frequencies, np.angle(ideal), legend_label="Ideal",
                          line_width=2, color='blue')
            phase_fig.line(frequencies, np.angle(corrupted), legend_label="Corrupted",
                          line_width=2, color='red', line_dash='dashed')
        ...
        for ant_id in antenna_ids:
            ...
            for t_idx, time in enumerate(times):
                for f_idx, freq in enumerate(frequencies):
                    jones_total[t_idx, f_idx] = simulator.compute_jones_matrix(freq, time, ant_id)
