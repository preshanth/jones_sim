...
def corrupt_ms_with_delays(
    ...
    for spw_id in range(n_spw):
        ...
        print(f"  SPW {spw_id}: {n_chan_spw} channels, {freq_min:.3f} - {freq_max:.3f} GHz")
    ...
    if delay_func is not None:
        ...
        antenna_delays = np.random.uniform(-delay_range_sec, delay_range_sec, n_antennas)
    ...
    for chunk_idx in range(n_chunks):
        ...
        print(f"\n{'─'*70}")
        print(f"Chunk {chunk_idx+1}/{n_chunks}: Rows {chunk_start:,} - {chunk_end:,} ({chunk_rows:,} rows)")
        print(f"{'─'*70}")
        ...
        try:
            ...
            data_chunk = table_tool.getcol("MODEL_DATA", startrow=chunk_start, nrow=chunk_rows)
            antenna1 = table_tool.getcol("ANTENNA1", startrow=chunk_start, nrow=chunk_rows)
            antenna2 = table_tool.getcol("ANTENNA2", startrow=chunk_start, nrow=chunk_rows)
            times = table_tool.getcol("TIME", startrow=chunk_start, nrow=chunk_rows)
            data_desc_ids = table_tool.getcol("DATA_DESC_ID", startrow=chunk_start, nrow=chunk_rows)
        ...
        for row_idx in range(n_row):
            ...
            for chan_idx in range(n_chan_spw):
                corrupted_data[:, chan_idx, row_idx] = corrupted_visibilities[vis_idx, :]
                vis_idx += 1
        ...
        try:
            ...
            table_tool.putcol(output_column, corrupted_data, startrow=chunk_start, nrow=chunk_rows)
            table_tool.close()
            print(f" Done")
        except Exception as e:
            print(f" Error: {e}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python corrupt_ms_clean.py <ms_path> [delay_ns] [ref_ant] [seed] [chunk_size] [batch_gpu_size] [--use-gpu]")
        print("\nExamples:")
