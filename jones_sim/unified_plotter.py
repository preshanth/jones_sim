...
from bokeh.models import HoverTool, Div, TabPanel, Tabs
...
class JonesPlotter:
    ...
    def __init__(self, sampler: JonesMCSampler, logger: Optional[logging.Logger] = None):
    ...
    def create_effect_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        for effect_name in self.sampler.config['effects'].keys():
            ...
            if effect_name == 'gains':
                ...
                try:
                    base_amp_summary = az.summary(self.sampler.trace, var_names=['base_amp_xx', 'base_amp_yy'])
                    thermal_summary = az.summary(self.sampler.trace, var_names=['thermal_amplitude'])

                    effect_summary.update({
                        'type': 'time_varying',
                        'base_amplitude_xx_mean': float(base_amp_summary.loc['base_amp_xx[0]', 'mean']),
                        'base_amplitude_xx_std': float(base_amp_summary.loc['base_amp_xx[0]', 'sd']),
                        'base_amplitude_yy_mean': float(base_amp_summary.loc['base_amp_yy[0]', 'mean']),
                        'base_amplitude_yy_std': float(base_amp_summary.loc['base_amp_yy[0]', 'sd']),
                        'thermal_amplitude_mean': float(thermal_summary.loc['thermal_amplitude[0]', 'mean']),
                        'thermal_timescale': self.sampler.config['effects']['gains']['thermal_timescale'],
                        'n_antennas_affected': self.sampler.n_antennas,
                        'time_range_hours': (self.sampler.times[-1] - self.sampler.times[0]) / 3600
                    })
                ...
                effect_summary.update({
                    ...
                    'frequency_range_mhz': (self.sampler.frequencies[-1] - self.sampler.frequencies[0]) / 1e6,
                    'jagged_amplitude': self.sampler.config['effects']['bandpass']['jagged_amplitude'],
                    'delay_std_ns': self.sampler.config['effects']['bandpass']['delay_std'] * 1e9,
                    'n_antennas_affected': self.sampler.n_antennas
                })

            elif effect_name == 'leakage':
                try:
                    leakage_summary = az.summary(self.sampler.trace, var_names=['leakage_hv', 'leakage_vh'])
                    effect_summary.update({
                        'type': 'static',
                        'hv_leakage_mean_magnitude': float(np.abs(leakage_summary.loc['leakage_hv[0]', 'mean'])),
                        'vh_leakage_mean_magnitude': float(np.abs(leakage_summary.loc['leakage_vh[0]', 'mean'])),
                        'configuration_amplitude': self.sampler.config['effects']['leakage']['amplitude'],
                        'n_antennas_affected': self.sampler.n_antennas
                    })
                except Exception as e:
                ...
                effect_summary.update({
                    ...
                    'rate_deg_per_hour': self.sampler.config['effects']['parallactic']['rate_deg_per_hour'],
                    'total_rotation_deg': self.sampler.config['effects']['parallactic']['rate_deg_per_hour'] *
                                         (self.sampler.times[-1] - self.sampler.times[0]) / 3600,
                    'time_range_hours': (self.sampler.times[-1] - self.sampler.times[0]) / 3600
                })
    ...
    def plot_gains_vs_time(self, antenna_id: int = 0, n_traces: int = 30) -> Tuple:
        ...
        gains_xx_flat = gains_xx.reshape(-1, self.sampler.n_antennas, self.sampler.n_times)
        gains_yy_flat = gains_yy.reshape(-1, self.sampler.n_antennas, self.sampler.n_times)
        ...
        for i in range(n_samples):
            amp_fig.line(times_hours, np.abs(gains_xx_ant[i]), alpha=0.1, color='blue', line_width=1)
            amp_fig.line(times_hours, np.abs(gains_yy_ant[i]), alpha=0.1, color='red', line_width=1)

            phase_fig.line(times_hours, np.degrees(np.angle(gains_xx_ant[i])),
                          alpha=0.1, color='blue', line_width=1)
            phase_fig.line(times_hours, np.degrees(np.angle(gains_yy_ant[i])),
                          alpha=0.1, color='red', line_width=1)

        # Plot percentiles
        amp_xx_perc = np.percentile(np.abs(gains_xx_ant), [5, 50, 95], axis=0)
        amp_yy_perc = np.percentile(np.abs(gains_yy_ant), [5, 50, 95], axis=0)
        phase_xx_perc = np.percentile(np.degrees(np.angle(gains_xx_ant)), [5, 50, 95], axis=0)
        phase_yy_perc = np.percentile(np.degrees(np.angle(gains_yy_ant)), [5, 50, 95], axis=0)

        # Confidence intervals
        amp_fig.varea(x=times_hours, y1=amp_xx_perc[0], y2=amp_xx_perc[2],
                     alpha=0.3, color='blue', legend_label='XX 90% CI')
        amp_fig.varea(x=times_hours, y1=amp_yy_perc[0], y2=amp_yy_perc[2],
                     alpha=0.3, color='red', legend_label='YY 90% CI')

        phase_fig.varea(x=times_hours, y1=phase_xx_perc[0], y2=phase_xx_perc[2],
                       alpha=0.3, color='blue', legend_label='XX 90% CI')
        phase_fig.varea(x=times_hours, y1=phase_yy_perc[0], y2=phase_yy_perc[2],
                       alpha=0.3, color='red', legend_label='YY 90% CI')

        # Medians
        amp_fig.line(times_hours, amp_xx_perc[1], color='darkblue', line_width=3, legend_label='XX median')
        amp_fig.line(times_hours, amp_yy_perc[1], color='darkred', line_width=3, legend_label='YY median')

        phase_fig.line(times_hours, phase_xx_perc[1], color='darkblue', line_width=3, legend_label='XX median')
        phase_fig.line(times_hours, phase_yy_perc[1], color='darkred', line_width=3, legend_label='YY median')

        # Configure
        for fig in [amp_fig, phase_fig]:
            fig.legend.location = "top_right"
            fig.add_tools(HoverTool(tooltips=[("Time", "@x{0.00} hr"), ("Value", "@y{0.000}")]))

        return amp_fig, phase_fig

    def plot_bandpass_vs_frequency(self, antenna_id: int = 0, n_traces: int = 30) -> Tuple:
        ...
        # Reshape and select antenna
        bp_amp_xx_flat = bp_amp_xx.reshape(-1, self.sampler.n_antennas, self.sampler.n_freqs)
        bp_amp_yy_flat = bp_amp_yy.reshape(-1, self.sampler.n_antennas, self.sampler.n_freqs)
        bp_phase_xx_flat = bp_phase_xx.reshape(-1, self.sampler.n_antennas, self.sampler.n_freqs)
        bp_phase_yy_flat = bp_phase_yy.reshape(-1, self.sampler.n_antennas, self.sampler.n_freqs)
        ...
        for i in range(n_samples):
            amp_fig.line(freqs_ghz, amp_xx_ant[i], alpha=0.1, color='blue', line_width=1)
            amp_fig.line(freqs_ghz, amp_yy_ant[i], alpha=0.1, color='red', line_width=1)

            phase_fig.line(freqs_ghz, np.degrees(phase_xx_ant[i]), alpha=0.1, color='blue', line_width=1)
            phase_fig.line(freqs_ghz, np.degrees(phase_yy_ant[i]), alpha=0.1, color='red', line_width=1)
        ...
        amp_fig.varea(x=freqs_ghz, y1=amp_xx_perc[0], y2=amp_xx_perc[2],
                     alpha=0.3, color='blue', legend_label='XX 90% CI')
        amp_fig.varea(x=freqs_ghz, y1=amp_yy_perc[0], y2=amp_yy_perc[2],
                     alpha=0.3, color='red', legend_label='YY 90% CI')

        phase_fig.varea(x=freqs_ghz, y1=phase_xx_perc[0], y2=phase_xx_perc[2],
                       alpha=0.3, color='blue', legend_label='XX 90% CI')
        phase_fig.varea(x=freqs_ghz, y1=phase_yy_perc[0], y2=phase_yy_perc[2],
                       alpha=0.3, color='red', legend_label='YY 90% CI')

        amp_fig.line(freqs_ghz, amp_xx_perc[1], color='darkblue', line_width=3, legend_label='XX median')
        amp_fig.line(freqs_ghz, amp_yy_perc[1], color='darkred', line_width=3, legend_label='YY median')

        phase_fig.line(freqs_ghz, phase_xx_perc[1], color='darkblue', line_width=3, legend_label='XX median')
        phase_fig.line(freqs_ghz, phase_yy_perc[1], color='darkred', line_width=3, legend_label='YY median')

        # Configure
        for fig in [amp_fig, phase_fig]:
            fig.legend.location = "top_right"
            fig.add_tools(HoverTool(tooltips=[("Frequency", "@x{0.000} GHz"), ("Value", "@y{0.000}")]))

        return amp_fig, phase_fig

    def plot_individual_jones_matrices(self, antenna_id: int = 0, time_idx: int = 0, freq_idx: int = 0,
                                     n_samples: int = 50) -> figure:
        """Plot individual Jones matrix elements as complex scatter plots [Confidence: 75% - Evidence: Complex plotting complexity]."""
        self.logger.info(f"Plotting individual Jones matrices for ant={antenna_id}, t={time_idx}, f={freq_idx}")
        ...
        try:
            ...
            if jones_samples.ndim == 5:  # All samples
                jones_at_point = jones_samples[:n_samples, antenna_id, time_idx, freq_idx, :, :]
            else:  # Single sample
                self.logger.warning("Only single sample available for Jones matrix plotting")
                return None
        ...
        for i in range(2):
            for j in range(2):
                ...
                fig.circle(element_data.real, element_data.imag,
                          size=8, alpha=0.6, color=colors[element_idx],
                          legend_label=element_names[element_idx])

        fig.legend.location = "top_right"
        fig.add_tools(HoverTool(tooltips=[("Real", "@x{0.000}"), ("Imag", "@y{0.000}")]))

        return fig

    def create_comprehensive_dashboard(self, output_file_name: str = "jones_dashboard.html") -> None:
        ...
        for effect_name, summary in summaries.items():
            ...
            if effect_name == 'gains':
                ...
                summary_html += f"<p><b>Channels:</b> {summary.get('n_channels', 0)}</p>"
            elif effect_name == 'leakage':
        ...
        if 'gains' in self.sampler.config['effects']:
            ...
            if gain_plots:
                gains_layout = gridplot([gain_plots[i:i+2] for i in range(0, len(gain_plots), 2)])
        ...
        if 'bandpass' in self.sampler.config['effects']:
            ...
            if bp_plots:
                bp_layout = gridplot([bp_plots[i:i+2] for i in range(0, len(bp_plots), 2)])
                bp_panel = TabPanel(child=bp_layout, title="Bandpass vs Frequency")
                tabs.append(bp_panel)

        # Individual Jones matrices
        jones_fig = self.plot_individual_jones_matrices(antenna_id=0, time_idx=0, freq_idx=0)
        if jones_fig:
