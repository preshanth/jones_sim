...
class JonesEquationGenerator:
    ...
    def _define_effect_matrices(self):
        ...
        self.effect_matrices["parallactic"] = Matrix(
            [[cos(self.psi), sin(self.psi)], [-sin(self.psi), cos(self.psi)]]
        )

        # Electronic gains (diagonal)
        self.effect_matrices["gains"] = Matrix([[self.g_xx, 0], [0, self.g_yy]])

        # Combined leakage and misalignment
        T_hv = self.d_hv + sp.tan(self.theta)
        T_vh = self.d_vh - sp.tan(self.theta)
        self.effect_matrices["leakage"] = Matrix([[1, T_hv], [T_vh, 1]])
        ...
        alpha_xx = (
            ...
            * (
                1 / self.nu
                + (sp.log(self.nu_min) - sp.log(self.nu_max))
        ...
        self.effect_matrices["tec"] = Matrix(
            [[exp(I * alpha_xx), 0], [0, exp(I * alpha_yy)]]
        )
        ...
        self.effect_matrices["crosshand_phase"] = Matrix(
            [[1, 0], [0, exp(I * self.phi)]]
        )

        # Identity matrix
        self.effect_matrices["identity"] = Matrix([[1, 0], [0, 1]])
    ...
    def generate_mueller_equation(
        self,
        antenna1_jones: Optional[Matrix] = None,
