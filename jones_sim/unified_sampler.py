...
from .effects import (
    ParallacticAngle, ElectronicGains, InstrumentalLeakage,
    BandpassDelay, RLDelayDifference, CrosshandPhase
)
...
class JonesMCSampler:
    ...
    def __init__(self, config: Union[Dict, str, Path]):
        ...
        self.frequencies = np.linspace(freq_config['start'], freq_config['end'], self.n_freqs)
    ...
    def build_unified_model(self) -> pm.Model:
        ...
        with pm.Model() as model:
            ...
            if 'gains' in self.config['effects']:
                ...
                base_amp_xx = pm.LogNormal('base_amp_xx',
                                          mu=np.log(gains_config['base_amplitude']),
                                          sigma=gains_config['amplitude_std'],
                                          shape=self.n_antennas)
                base_amp_yy = pm.LogNormal('base_amp_yy',
                                          mu=np.log(gains_config['base_amplitude'] * 0.95),
                                          sigma=gains_config['amplitude_std'],
                                          shape=self.n_antennas)

                # Thermal drift parameters
                thermal_amp = pm.Normal('thermal_amplitude',
                                       mu=0.0, sigma=gains_config['thermal_amplitude'],
                                       shape=self.n_antennas)
                thermal_timescale = gains_config['thermal_timescale']

                # Phase drift rates
                phase_drift_xx = pm.Normal('phase_drift_xx',
                                          mu=0.0, sigma=gains_config['phase_drift_std'],
                                          shape=self.n_antennas)
                phase_drift_yy = pm.Normal('phase_drift_yy',
                                          mu=0.0, sigma=gains_config['phase_drift_std'],
                                          shape=self.n_antennas)
                ...
                amp_yy_time = base_amp_yy[:, None] + thermal_mod * 0.8  # Correlated but different
                ...
                gains_xx = pm.Deterministic('gains_xx',
                                           amp_xx_time * pt.exp(1j * phase_xx_time))
                gains_yy = pm.Deterministic('gains_yy',
                                           amp_yy_time * pt.exp(1j * phase_yy_time))
            ...
            if 'bandpass' in self.config['effects']:
                ...
                cable_delay = pm.Normal('cable_delay',
                                       mu=0.0, sigma=bp_config['delay_std'],
                                       shape=self.n_antennas)

                # Jagged amplitude per channel
                jagged_amp_xx = pm.Normal('jagged_amp_xx',
                                         mu=0.0, sigma=bp_config['jagged_amplitude'],
                                         shape=(self.n_antennas, self.n_freqs))
                jagged_amp_yy = pm.Normal('jagged_amp_yy',
                                         mu=0.0, sigma=bp_config['jagged_amplitude'],
                                         shape=(self.n_antennas, self.n_freqs))
                ...
                # Cable delay phase
                cable_phase = 2 * np.pi * cable_delay[:, None] * (freq_tensor[None, :] - ref_freq)

                # Total bandpass amplitude (log-space for positivity)
                bp_amp_xx = pm.Deterministic('bandpass_amp_xx',
                                            pt.exp(jagged_amp_xx))
                bp_amp_yy = pm.Deterministic('bandpass_amp_yy',
                                            pt.exp(jagged_amp_yy))

                # Total bandpass phase
                bp_phase_xx = pm.Deterministic('bandpass_phase_xx', cable_phase)
                bp_phase_yy = pm.Deterministic('bandpass_phase_yy', cable_phase * 1.1)
            ...
            if 'leakage' in self.config['effects']:
                ...
                d_hv_real = pm.Normal('d_hv_real', mu=0.0, sigma=leak_config['amplitude'],
                                     shape=self.n_antennas)
                d_hv_imag = pm.Normal('d_hv_imag', mu=0.0, sigma=leak_config['amplitude'],
                                     shape=self.n_antennas)
                d_vh_real = pm.Normal('d_vh_real', mu=0.0, sigma=leak_config['amplitude'],
                                     shape=self.n_antennas)
                d_vh_imag = pm.Normal('d_vh_imag', mu=0.0, sigma=leak_config['amplitude'],
                                     shape=self.n_antennas)

                leakage_hv = pm.Deterministic('leakage_hv', d_hv_real + 1j * d_hv_imag)
                leakage_vh = pm.Deterministic('leakage_vh', d_vh_real + 1j * d_vh_imag)
            ...
            if 'parallactic' in self.config['effects']:
                ...
                para_rate = para_config['rate_deg_per_hour'] * np.pi / 180 / 3600  # rad/sec
                para_angles = para_rate * self.times  # Simple linear model
                para_tensor = pm.Deterministic('parallactic_angles',
                                              pt.as_tensor_variable(para_angles))
    ...
    def compute_jones_matrices(self,
        ...
        if sample_idx is not None:
            # Single sample
            n_samples = 1
            slice_obj = slice(sample_idx, sample_idx + 1)
        else:
            # All samples
            chain_samples = self.trace.posterior.dims['draw'] * self.trace.posterior.dims['chain']
            ...
            for ant in range(self.n_antennas):
                for t_idx in range(self.n_times):
                    for f_idx in range(self.n_freqs):
                        ...
                        if 'parallactic' in self.config['effects']:
                            psi = self.trace.posterior['parallactic_angles'].values.flatten()[t_idx]
                            P = np.array([[np.cos(psi), np.sin(psi)],
                                         [-np.sin(psi), np.cos(psi)]])
                            J = J @ P

                        if 'leakage' in self.config['effects']:
                            hv = self.trace.posterior['leakage_hv'].values.flatten()[ant]
                            vh = self.trace.posterior['leakage_vh'].values.flatten()[ant]
                            X = np.array([[1, hv], [vh, 1]])
                            J = J @ X

                        if 'gains' in self.config['effects']:
                            g_xx = self.trace.posterior['gains_xx'].values.flatten()[ant * self.n_times + t_idx]
                            g_yy = self.trace.posterior['gains_yy'].values.flatten()[ant * self.n_times + t_idx]
                            G = np.array([[g_xx, 0], [0, g_yy]])
                            J = J @ G

                        if 'bandpass' in self.config['effects']:
                            amp_xx = self.trace.posterior['bandpass_amp_xx'].values.flatten()[ant * self.n_freqs + f_idx]
                            amp_yy = self.trace.posterior['bandpass_amp_yy'].values.flatten()[ant * self.n_freqs + f_idx]
                            phase_xx = self.trace.posterior['bandpass_phase_xx'].values.flatten()[ant * self.n_freqs + f_idx]
                            phase_yy = self.trace.posterior['bandpass_phase_yy'].values.flatten()[ant * self.n_freqs + f_idx]

                            B = np.array([[amp_xx * np.exp(1j * phase_xx), 0],
...
def main():
    ...
    parser.add_argument('--output', type=str, default='jones_samples.nc',
    ...
    parser.add_argument('--plot', action='store_true',
                       help='Generate interactive plots dashboard')
    parser.add_argument('--plot-output', type=str, default='jones_dashboard.html',
    ...
    print(f"Running MCMC sampling: {sampling_config['draws']} draws, {sampling_config['chains']} chains")
    ...
    if args.plot:
        ...
        for effect_name, summary in summaries.items():
            ...
            if 'error' in summary:
                ...
                print(f"    Base amplitude XX: {summary.get('base_amplitude_xx_mean', 0):.3f}")
                print(f"    Thermal timescale: {summary.get('thermal_timescale', 0)/3600:.1f} hours")
            elif effect_name == 'bandpass':
                print(f"    Frequency range: {summary.get('frequency_range_mhz', 0):.1f} MHz")
