    ...
    def test_complete_jones_workflow(self):
        ...
        config = {
            ...
            "effects": {
                ...
                "bandpass": {"delay_std": 5e-10, "jagged_amplitude": 0.03},  # 0.5 ns
                "leakage": {"amplitude": 0.001},
                "parallactic": {"rate_deg_per_hour": 15.0},
            },
            "sampling": {"draws": 500, "tune": 200, "chains": 2, "target_accept": 0.85},
        }
        ...
        logger.info(
            f"Sampler initialized: {sampler.n_antennas} antennas, {sampler.n_times} times, {sampler.n_freqs} frequencies"
        )
        ...
        for effect in config["effects"].keys():
            if effect == "gains":
                assert any(
                    "base_amp" in var for var in model_vars
                ), "Missing gains variables"
            elif effect == "bandpass":
                assert any(
                    "cable_delay" in var for var in model_vars
                ), "Missing bandpass variables"
            elif effect == "leakage":
                assert any(
                    "d_hv" in var or "d_vh" in var for var in model_vars
                ), "Missing leakage variables"
        ...
        logger.info(
            f"Sampling completed: {trace.posterior.dims['draw']} draws x {trace.posterior.dims['chain']} chains"
        )
        ...
        for effect_name, summary in summaries.items():
            ...
            if "error" not in summary:
                if effect_name == "gains":
                    logger.info(
                        f"    Base amplitude XX: {summary.get('base_amplitude_xx_mean', 0):.3f} ± {summary.get('base_amplitude_xx_std', 0):.3f}"
                    )
                    logger.info(
                        f"    Thermal timescale: {summary.get('thermal_timescale', 0)/60:.1f} minutes"
                    )
                    ...
                    logger.info(
                        f"    HV leakage magnitude: {summary.get('hv_leakage_mean_magnitude', 0):.4f}"
                    )
                elif effect_name == "parallactic":
                    logger.info(
                        f"    Total rotation: {summary.get('total_rotation_deg', 0):.1f} degrees"
                    )
        ...
        assert os.path.exists(
            dashboard_file
        ), f"Dashboard file not created: {dashboard_file}"
        ...
        assert os.path.exists(
            summary_json_file
        ), f"Summary JSON not created: {summary_json_file}"
        ...
        if "gains" in final_summaries and "error" not in final_summaries["gains"]:
            ...
            assert (
                0.3 < base_amp < 3.0
            ), f"Unreasonable base amplitude: {base_amp:.3f} (should be 0.3-3.0)"

        if "bandpass" in final_summaries:
            n_channels = final_summaries["bandpass"].get("n_channels", 0)
            assert (
                n_channels == config["grid"]["n_frequencies"]
        ...
        try:
            ...
            subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "from jones_sim.unified_sampler import main; import sys; sys.argv = ['jones-sim', '--create-default-config', 'cli_default.json']; main()",
                ],
                capture_output=True,
                text=True,
                timeout=30,
