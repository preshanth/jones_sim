    ...
    def test_create_full_example(self, tmp_path):
        ...
        config = {
            ...
            "effects": {
                ...
                "leakage": {"amplitude": 0.002},
                "parallactic": {"rate_deg_per_hour": 15.0},
            },
            "sampling": {"draws": 200, "tune": 100, "chains": 2, "target_accept": 0.85},
        }
        ...
        try:
            ...
            print(
                f"Expected: ({sampler.n_antennas}, {sampler.n_times}, {sampler.n_freqs}, 2, 2)"
            )

            # Verify Jones matrices are reasonable
            assert jones_first.shape == (
                sampler.n_antennas,
                sampler.n_times,
                sampler.n_freqs,
                2,
            ...
            assert np.all(
                np.abs(diag_elements) > 0.1
            ), "Jones matrix diagonal elements too small"
            ...
            print(
                "This might be due to complex PyMC variable extraction - needs debugging"
            )
